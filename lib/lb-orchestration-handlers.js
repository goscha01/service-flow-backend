...
function makeBusyHandler({ supabase, logger, resolveLocationForOrchestrationScope }) {
  return async function busyHandler(req, res) {
    ...
    // Location scope resolution — only when the caller supplied an
    // external_business_id. Fail-closed on unresolved: returning
    // tenant-wide busy in that case would surface sibling-location
    // jobs to the AI, which is precisely what per-account scoping is
    // meant to prevent (Karen D. / K&D multi-location tenants).
    ...
    try {
      let query = supabase
        .from('jobs')
        .select('id, scheduled_date, end_time, duration')
        .eq('user_id', userId)
