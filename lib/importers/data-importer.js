...
async function importLeads(supabase, userId, rows, settings, sendProgress) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      const leadCost = toNumber(pick(row, ['leadCost', 'opportunity_cost', 'lead_cost', 'Lead Cost', 'CPL', 'Acquisition Cost', 'Cost']));
      ...
      if (existingId) {
        if (updateExisting) {
          const { error: upErr } = await supabase.from('opportunities').update(payload).eq('id', existingId);
          if (upErr) result.errors.push(`Row ${i + 1}: ${upErr.message}`);
      ...
      if (leadId) {
        ...
        if (taskRows.length > 0) {
          const { error: taskErr } = await supabase.from('opportunity_tasks').insert(taskRows);
          if (taskErr) {
...
// 'leads' retained for backward-compat with existing import_mapping_presets
// rows and any client still POSTing the legacy target name.
const SUPPORTED_TYPES = ['customers', 'opportunities', 'leads', 'jobs', 'team_members', 'services', 'territories', 'reviews'];
