...
async function importLeads(supabase, userId, rows, settings, sendProgress) {
  ...
  if (!pipelines || pipelines.length === 0) {
    result.errors.push('No pipelines exist for this account. Create one before importing leads.');
    return result;
  }
  const pipelineId = (pipelines.find((p) => p.is_default) || pipelines[0]).id;
  ...
  if (!stages || stages.length === 0) {
    result.errors.push(`Pipeline ${pipelineId} has no stages. Add at least one stage before importing leads.`);
    return result;
  }
...
const SUPPORTED_TYPES = ['customers', 'leads', 'jobs', 'team_members', 'services', 'territories'];
