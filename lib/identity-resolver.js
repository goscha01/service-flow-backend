  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (phoneCands.length > 0) {
      ...
      if (strong.length === 0 && classified.length === 1 && nameCompat.length === 1) {
        // Runtime: phone-only-no-conflict is OK. Strict (backfill): reject —
        // never merge on phone alone UNLESS the incoming signal has no name
        // at all. In that case there's nothing to conflict with (tiny risk of
        // household phone mixups, but the attaching is a conversation-level
        // association, not a CRM rename, and can be split later if needed).
        const attemptedHasNoName = !normalized_name || !normalized_name.trim();
        if (strict && !attemptedHasNoName) {
