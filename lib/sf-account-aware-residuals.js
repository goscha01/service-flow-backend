...
// ─── 5-bucket terminal taxonomy ────────────────────────────────────────────
//
// Per operator policy 2026-06-06: known-Account non-primary inquiries are
// NEVER marked no_match on LB. They get safe terminal classifications that
// preserve the row for attribution/history and platform analytics.
//
// AUTO_LINK                  — existing customer/account with missing reconciliation
// NEEDS_REVIEW               — ambiguous account ownership; operator must decide
// REPEAT_INQUIRY_SAME_ACCOUNT— same Account, separate inquiry; Account already
//                              represented in SF/LB; preserve for history
// MARKETPLACE_DUPLICATE      — TT/Yelp distributed the same customer action;
//                              preserve for platform analytics
// NO_MATCH                   — no Account, no Lead, no Customer relationship
// FALLBACK_TO_MATCHER        — out of PR D scope; existing matcher decides
const ACTIONS = Object.freeze({
  AUTO_LINK:                  'auto_link',
  NEEDS_REVIEW:               'needs_review',
  REPEAT_INQUIRY_SAME_ACCOUNT:'repeat_inquiry_same_account',
  MARKETPLACE_DUPLICATE:      'marketplace_duplicate',
  NO_MATCH:                   'no_match',
  FALLBACK_TO_MATCHER:        'fallback_to_matcher',
});

// LB-safe terminal delivery paths.
//
// SYNC_APPLY            — existing runHistoricalSyncApply (writes SF + LB linked)
// FEEDBACK              — existing runHistoricalFeedbackApply, LB-only:
//                         emits syncStatus='needs_review' (NEVER no_match for
//                         known accounts)
// LB_TERMINAL_SKIPPED   — new LB-side path: writes syncStatus='skipped' +
//                         explicit syncReason (e.g. 'sf_terminal:repeat_inquiry'
//                         or 'sf_terminal:marketplace_duplicate'). Requires a
//                         small LB applyBulkLink addition (separate PR). Until
//                         then this is dry-run / report-only.
// NONE                  — no LB write; fallback to existing matcher
const DELIVERY_PATHS = Object.freeze({
  SYNC_APPLY:           'sync_apply',
  FEEDBACK:             'feedback',
  LB_TERMINAL_SKIPPED:  'lb_terminal_skipped',
  NONE:                 null,
});

// syncReason values for LB_TERMINAL_SKIPPED writes. Used as the second
// segment of `sf_terminal:<reason>` so LB telemetry can distinguish causes.
...
function classifyAccountAware({ lbCandidate, categorized, accountContext, options }) {
  ...
  if (reason === 'already_reconciled_customer') {
    ...
    return {
      ...
      action:               ACTIONS.REPEAT_INQUIRY_SAME_ACCOUNT,
      delivery_path:        DELIVERY_PATHS.LB_TERMINAL_SKIPPED,
      lb_terminal_reason:   TERMINAL_REASONS.REPEAT_INQUIRY_SAME_ACCOUNT,
      reason:               'Reconciled customer; this is a separate inquiry from the same person — terminal skip, preserve for history (not no_match)',
    };
  }
  ...
  if (reason === 'cross_inquiry_or_non_lb_sf_lead') {
    ...
    if (siblingMarketplaceDup) {
      return {
        ...
        action:             ACTIONS.MARKETPLACE_DUPLICATE,
        delivery_path:      DELIVERY_PATHS.LB_TERMINAL_SKIPPED,
        lb_terminal_reason: TERMINAL_REASONS.MARKETPLACE_DUPLICATE,
        reason:             'sibling LB lead created within ' + DUPLICATE_WINDOW_SECONDS + 's — platform duplicate, preserve for analytics (not no_match)',
      };
    }
    if (acctHasPaid) {
      ...
      return {
        ...
        delivery_path:      DELIVERY_PATHS.LB_TERMINAL_SKIPPED,
        lb_terminal_reason: TERMINAL_REASONS.REPEAT_INQUIRY_SAME_ACCOUNT,
        reason:             'Account has paid customer; same person, different inquiry — terminal skip, preserve for history (not no_match)',
      };
    }
    if (siblingLinked) {
      return {
        ...
        delivery_path:      DELIVERY_PATHS.LB_TERMINAL_SKIPPED,
        lb_terminal_reason: TERMINAL_REASONS.REPEAT_INQUIRY_SAME_ACCOUNT,
        reason:             'sibling LB lead already linked to SF; this row is the un-linked twin — terminal skip, preserve for history (not no_match)',
      };
    }
...
async function runAccountAwareResidualCleanupDryRun(supabase, args) {
  ...
  const tally = {
    ...
    repeat_inquiry_same_account:  { lb_terminal_skipped: 0 },
    marketplace_duplicate:        { lb_terminal_skipped: 0 },
    no_match:                     { feedback:            0 },
  ...
  for (const lbCandidate of candidates) {
    ...
    classified.push({
      ...
      lb_terminal_reason: cls.lb_terminal_reason || null,
    ...
    else if (cls.action === ACTIONS.REPEAT_INQUIRY_SAME_ACCOUNT)   tally.repeat_inquiry_same_account.lb_terminal_skipped++;
    else if (cls.action === ACTIONS.MARKETPLACE_DUPLICATE)         tally.marketplace_duplicate.lb_terminal_skipped++;
    else if (cls.action === ACTIONS.NO_MATCH)                      tally.no_match.feedback++;
