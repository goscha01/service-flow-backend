...
const { deriveSourceForRow, deriveSourceFieldsForRow } = require('./customer-source-fill');
...
async function fillMissingAttribution(supabase, userId, { logger } = {}) {
  ...
  // Helper — fill null-source rows for a given table. When table === 'leads',
  // also fills leads.source_raw with the raw OP company tag (migration 050,
  // two-field attribution). For 'customers' table, single-field fill is
  // preserved (customers.source_raw not introduced in migration 050).
  async function fillTable(table) {
    ...
    while (true) {
      ...
      // Derive + write per row. Leads use two-field model (source + source_raw);
      // customers keep single-field model until customers.source_raw exists.
      for (const row of rows) {
        const p10 = normalizePhone(row.phone);
        if (!p10) continue;
        const candidates = convsByPhone[p10] || [];
        if (table === 'opportunities') {
          const derived = deriveSourceFieldsForRow(row, sourceMappings, candidates);
          if (!derived) continue;
          await supabase.from(table)
            .update({ source: derived.source, source_raw: derived.source_raw, updated_at: new Date().toISOString() })
  ...
  result.leads_filled = await fillTable('opportunities');
...
async function runIntegrationSync(supabase, userId, source, opts = {}) {
  ...
  try {
    ...
    // 6b: recover stuck identities — floating, no SF lead, but with phone +
    // name + Company tag from OP. Bug 841fd8a: LB sometimes ingested the
    // contact (set leadbridge_contact_id) but never produced a lead. The
    // OP path now correctly recovers, but only fires on new webhook events;
    // running this step inside Sync Now retroactively unsticks them.
    if (typeof deps.recreateOpLeads === 'function') {
      try {
        const r = await deps.recreateOpLeads(userId);
        summary.leads_recovered = r?.created || 0;
      } catch (e) {
        if (logger?.error) logger.error(`[SyncOrchestrator:${source}] recreateOpLeads failed`, e);
        summary.errors.push(`recreateOpLeads: ${e?.message || e}`);
      }
    }
