...
async function fillNormalizedNamesIdentities(supabase, userId, { progress } = {}) {
  ...
  // Don't increment offset — the WHERE normalized_name IS NULL filter
  // shrinks as we write, so repeatedly fetching page 0 drains the set.
  // Offset-based pagination would skip rows whose offset shifted.
  while (true) {
    ...
    // If nothing was written this page (all skipped_no_name), the same rows
    // would come back forever — break to avoid infinite loop.
    if (!wroteAny) break;
  }
  return counts;
}
