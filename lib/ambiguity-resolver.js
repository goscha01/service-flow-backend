...
function buildMergePatch(target, ambiguity) {
  ...
  // reclassify this identity. status='manual' is honored by the resolver.
...
function buildCreateFromAmbiguity(ambiguity) {
  ...
  const row = {
    ...
    // Operator explicitly created this as a separate identity; don't let
    // the runtime resolver reclassify it.
    status: 'manual',
    identity_priority_source: ambiguity.source === 'zenbooker' ? 'sync' : ambiguity.source,
  };
